...
for line in f0:
	...
	if dest == '[]':
		edges.append((source,-1)) # for isolated nodes, create special destination -1 and check it later.
		continue
...
if len(argv)==2:
	...
	for j in range(len(edges)):
		if edges[j][1] == -1: # If isolated node, print it as it is
			treeString += '"' + str(edges[j][0]) + '"\n'
		else:
